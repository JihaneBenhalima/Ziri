<?php
    ...
    $records_per_page = 12; // the number of properties displayed per page
    ...
    {
     $current_page = 1; // If "page" parameter is not set or not numeric, set $current_page to 1 (default)
    }
    // SQL query to retrieve total number of records
    $total_query = $connect->query("SELECT COUNT(*) as total_records FROM property "); // Execute a query to count the total number of properties in the "property" table
    $total_results = $total_query->fetch(PDO::FETCH_ASSOC)['total_records'];
    $total_pages = ceil($total_results / $records_per_page);// Calculate the total number of pages required to display all properties
    // SQL query to retrieve records for current page
    $start = ($current_page - 1) * $records_per_page; // Calculate the starting index of properties for the current page
    $users_query = $connect->prepare("SELECT * FROM property LIMIT :start, :records_per_page");
             ...
             <?php foreach($properties as $property):?>
                 <div class="property-box"  id="property-<?= $property->id ?>">
                     ...
                     <div class="property-box-overlay">
                         <a class="fancy" href="../GuestSession/propertyDetails_guest.php?PropertyMoreDetails=<?php echo $property->id ?>">

<?php
 ...
 $id = $_GET['profile'];
 $agentProfile = getUserProfile($id);
 ...
 if ($agentProfile)
 {
  ...
  if(isset($_POST['addAgentInfos']))
  {
   ...
   if($phoneNumber=='' or $agreement=='' or $profilePicture=='')
   {
     $empty=1;
   }
   else
   {
     ...
     { 
       require_once("../Configuration/command.php");
       addUserInfos($phoneNumber, $agreement, $profilePicture, $id );
       $added=1;
      }
      catch(Exception $e)
      {
 ...
 $query = "SELECT profilePicture,password FROM user WHERE user_type='user' AND id = :id";
 $stmt = $connect->prepare($query);
 $stmt->bindValue(':id', $id, PDO::PARAM_INT); // Bind the ID parameter
 $stmt->execute();
 $result = $stmt->fetch(PDO::FETCH_ASSOC);
 $profileEdit=getUserProfile($id);
 if (is_array($profileEdit) || is_object($profileEdit)) {
  ...
  // Handle the case when $profileEdit is not an array or object
  echo "Invalid data type for \$profileEdit. Expected array or object.";
  echo "Actual type: " . gettype($profileEdit);
}
 ...
 if(isset($_POST['edit_profile']))
 {
   ...
   $temp_profilePicture = $_FILES['profilePicture']['tmp_name'];
   //checking empty condition
   if($name=='' or $last_name=='' or $email=='' or $phoneNumber=='' )
   {
   ...
   { 
     ...
     try
     {
       EditProfileUser($name, $last_name, $email, $phoneNumber, $hashedPassword, $profilePicture, $id);
...
<html lang="en">
 ...
 <body>
   ...
   ?>
   <main id="main"> 
     <section><!--This section if for the agent to add their infos so they can post-->
     <div class="my-additional-infos" style="margin-top: 50px;margin-bottom:50px;<?php if(!is_null($newProfile) && !empty($newProfile->phoneNumber)){echo "display:none !important;";}   ?>">
       <h2 style="margin-bottom: 15px;text-align:center;">Hello,<?php echo $_SESSION['user_name']?>&nbsp;<?php echo $_SESSION['user_last_name']?></h2>
       <p style="margin-bottom: 20px;text-align:center;">Please fill this additional infos in order to complete your profile<br>And benefit of full functionalities</p>
       <div class="modal">
       ...
       ?>
           ...
           <div class="agent-supp-info">
             <div class="input_container-p">
                ...
                <div style="justify-content: center;display: flex;">
                 <div id="image-preview" style="background-image: url(../img/download.jpg);"></div>
                </div>
             </div>
             <div class="input_container-p">
           ...
           <button name="addAgentInfos" type="submit" class="add-infos" style="cursor:pointer;">Submit</button>
         </form>
       </div>
     </div>
     </section>
     ...
     <section style="margin-top:30px;">
       <div class="container" <?php if(!is_null($newProfile) && empty($newProfile->phoneNumber)){echo "<div class=\"container\" style=\"display:none !important;\">";}?>>
         <h2>My account</h2>
                ...
                <div class="fields">
                  <div class="input-field">
                    <div class="profile-pic-div">
                      ...
                      <input name="profilePicture" type="text" value="<?= $id ?>">
